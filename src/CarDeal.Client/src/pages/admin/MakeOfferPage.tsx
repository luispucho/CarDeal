...
export default function MakeOfferPage() {
  ...
  const offerMutation = useMutation({
    ...
    onError: (err: any) => setError(err.response?.data?.error || t('adminOffer.failedOffer')),
  });
  ...
  const consignMutation = useMutation({
    ...
    onError: (err: any) => setError(err.response?.data?.error || t('adminOffer.failedConsignment')),
  });
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Link to={`/admin/cars/${id}`} className="text-blue-600 hover:underline text-sm">{t('adminOffer.backToReview')}</Link>
      <h1 className="text-2xl font-bold mt-2 mb-1">{t('adminOffer.makeOfferConsign')}</h1>
      <p className="text-gray-500 mb-6">{car.year} {car.make} {car.model} — {t('adminOffer.asking')} {car.askingPrice ? `$${car.askingPrice.toLocaleString()}` : t('adminOffer.na')}</p>
      ...
      {tab === 'offer' ? (
        <form onSubmit={offerForm.handleSubmit((d) => offerMutation.mutate(d))} className="bg-white rounded-xl shadow-sm p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminOffer.offerAmount')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminOffer.notes')}</label>
            <textarea {...offerForm.register('notes')} rows={3} className="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder={t('adminOffer.notesPlaceholder')} />
          </div>
          <button type="submit" disabled={offerMutation.isPending} className="w-full bg-green-600 text-white py-3 rounded-lg hover:bg-green-700 disabled:opacity-50 transition font-semibold">
            {offerMutation.isPending ? t('adminOffer.submittingOffer') : t('adminOffer.submitOffer')}
          </button>
        </form>
      ) : (
        <form onSubmit={consignForm.handleSubmit((d) => consignMutation.mutate(d))} className="bg-white rounded-xl shadow-sm p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminOffer.agreedPrice')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminOffer.commission')}</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminOffer.startDate')}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('adminOffer.endDate')}</label>
          ...
          <button type="submit" disabled={consignMutation.isPending} className="w-full bg-indigo-600 text-white py-3 rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition font-semibold">
            {consignMutation.isPending ? t('adminOffer.creatingConsignment') : t('adminOffer.createConsignment')}
          </button>
        </form>
      )}
    </div>
